...
interface Action {
  ...
  description: string;
  icon: string;
  onClick: (filePath: string, e: PointerEvent) => boolean | Promise<boolean> | void;
}
...
const fileActions: Action[] = [
  ...
  },
  {
    ...
    onClick: async (filePath, e) => {
      let fullUrl: string = filePath.startsWith("/")
  ...
  },
  {
    ...
    onClick: async (filePath, e) => {
      const rawPath = filePath.replace("/blob/", "/commits/");
      const rawUrl = `https://github.com${rawPath}`;
  ...
  },
  {
    ...
    name: "Copy Content",
    description:
      "Copy the content of the file to clipboard (only for small files).",
...
function isFolder(fileRow: HTMLElement): boolean {
  const href =
    fileRow.querySelector("a.Link--primary")?.getAttribute("href") ?? "";
...
function findInjectPoints(fileRow: HTMLElement): HTMLElement[] | null {
  const injectionPoint = fileRow.querySelector(
  ...
  const wideScreenInjectionPoint = fileRow.querySelector(
  ...
  return [
    injectionPoint as HTMLElement,
    wideScreenInjectionPoint as HTMLElement,
  ];
}
...
function createQuickActionsContainer(fileRow: HTMLElement): HTMLElement {
  ...
  const MVP_ACTIONS = rowIsFolder
    ? folderActions.filter((a) => MVP_FOLDER_ACTIONS.includes(a.id))
    : fileActions.filter((a) => MVP_FILE_ACTIONS.includes(a.id));
...
function handleRenderRequest(fileRow: Element, event: MouseEvent) {
  ...
  injectionPoints.forEach((point, index) => {
    if (!point.querySelector(".gitindex-quick-actions-container")) {
      ...
      LOG.log(
        `Injection point ${index} already has a container, skipping injection.`,
      );

      const existingContainer = point.querySelector(
        ".gitindex-quick-actions-container",
      ) as HTMLElement;
...
function injectEventListeners() {
  ...
  fileRows.forEach((row) => {
    ...
    const handler: EventListener = (event) =>
      handleRenderRequest(row, event as MouseEvent);
    const removeHandler: EventListener = (event) =>
      removeExistingContainers(event as MouseEvent);
